...
int n, p, s, a[3003], b[3003], src, snk;
...
int main() {
	...
	src = n + 3;
	snk = n + 4;
	addEdge(src, p_team, p, 0);
	addEdge(src, s_team, s, 0);
	for (int i = 1; i <= n; i++) {
		int x;
		cin >> x;
		addEdge(p_team, i, 1, -x);
	}
	for (int i = 1; i <= n; i++) {
		int x;
		cin >> x;
		addEdge(s_team, i, 1, -x);
	}
	for (int i = 1; i <= n; i++) addEdge(i, snk, 1, 0);
	cout << -get_flow().se << '\n';
	for (auto id : adj[p_team]) if (edges[id].to <= n && edges[id].flow == edges[id].capa) cout << edges[id].to << ' ';
	cout << '\n';
	for (auto id : adj[s_team]) if (edges[id].to <= n && edges[id].flow == edges[id].capa) cout << edges[id].to << ' ';
}
