...
priority_queue < pair < int, ll >, vector < pair < int, ll > >, greater < pair < int, ll > > > heap;

bool check(ll x) {
	while (heap.size()) heap.pop();
	for (int i = 1; i < SZE(compress); i++) {
		ll rem = calc(compress[i] - compress[i - 1] - 1, x);
		while (heap.size()) {
		...
		rem = x;
		for (auto nxt : st[i]) heap.push(nxt);
		while (heap.size()) {
			auto u = heap.top(); heap.pop();
			if (u.se > rem) {
				u.se -= rem;
		...
		if (heap.size() && heap.top().fi <= i) return false;
	}
...
int main() {
	...
	//if (n <= 500 && m <= N) sub2::solve();
	compress.pb(0);
	for (int i = 1; i <= n; i++) compress.pb(a[i].s), compress.pb(a[i].t);
