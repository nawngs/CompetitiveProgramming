...
int n, m, dist[18][1502], dist_rev[18][1502], f[18], dp[18][18][(1 << 17) - 1], q, x, y;

vector < pii > adj[1502], adj_rev[1502];
...
void solve() {
		...
		for (int bit = 0; bit < (1 << m); bit ++) {
			...
			for (int finish = 0; finish < m; finish ++) {
				...
				for (int pre = 0; pre < m; pre ++) {
					...
					dp[i][finish][bit] = min(dp[i][finish][bit], dp[i][pre][tam] + dist[pre][f[finish]]);
				}
			}
 		}
	}
	while (q --) {
		...
		int ans = INF;
		for (int i = 0; i < m; ++i) 
			for (int j = 0; j < m; ++j) ans = min(ans, dist_rev[i][x] + dp[i][j][(1 << m) - 1] + dist[j][y]);
