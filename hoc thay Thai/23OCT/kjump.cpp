	...
	while (q--) {
		...
		if (t == 1) {
			...
			if (k == 0) cout << i << '\n';
				...
				for (int j = i + 1; j < (i / BLOCK + 1) * BLOCK; j++) 
					if (k && a[j] > a[i] && j - i <= 100) i = j, k --;
				cout << i << '\n';
			}
		}
		else {
			...
			for (int i = idl + 1; i < idr; i++) chg[i] += d;
			for (int i = l; i < min(r + 1, (idl + 1) * BLOCK); i++) a[i] += d;
			if (idl != idr) for (int i = idr * BLOCK; i <= r; i++) a[i] += d;
			// for (int i = 1; i <= n; i++) cerr << a[i] << " ";
			// cerr << '\n';
			upd(idl - 1, idl + 1); upd(idr - 1, idr + 1);
		}
	}
}