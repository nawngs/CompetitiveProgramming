...
int n, q, e[nmax + 3], cnt_child[nmax + 3], par[nmax + 3][20], log2_n, h[nmax + 3];
...
int query_tree(int u, int a) {
	...
	while (1) {	
		if (aid == uid) {
			res ^= query(1, 1, n, pos_it[a], pos_it[u]);
			break;
		}
		res ^= query(1, 1, n, pos_it[chain_head[uid]], pos_it[u]);
...
int main() {
	...
	hld();
	init();
	for (int i = 1; i <= n; i++) update(1, 1, n, pos_it[i], e[i]);
	while (q--) {
		int type;
		cin >> type;
		if (type == 1) {
			...
			cin >> u >> v;
			int a = LCA(u, v);
			cout << (query_tree(u, a) ^ query_tree(v, a) ^ e[a]) << '\n';
		}
	}	
}
